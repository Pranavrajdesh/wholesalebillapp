...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .filters { display: flex; gap: 8px; flex-wrap: wrap; align-items: flex-end; margin: 10px 0 14px; }
        ...
        .dctag { display: inline-block; padding: 1px 7px; border-radius: 3px; font-size: 9.5px; font-weight: 700; letter-spacing: 0.5px; vertical-align: 1px; margin-left: 6px; }
        .dctag.dr { color: #b00020; border: 1px solid #b00020; background: #fdf3f3; }
        .dctag.cr { color: #1e7e34; border: 1px solid #1e7e34; background: #e6f4ea; }
        .muted { color: #666; }
...
<body>
    <div class="topbar">
        @if (!$public)
            ...
            <a class="tbtn outline" href="{{ route('partners.index') }}">ALL PARTNERS</a>
    ...
    <div class="sheet">
        ...
        <div class="party">
            ...
            <div>Mobile: {{ $partner->mobile }}</div>
            @if ($partner->gst_number)<div>GSTIN: {{ $partner->gst_number }}</div>@endif
        </div>
        ...
        @if (!$public)
            <div class="filterbox">
                ...
                <div class="filters frow last">
                    ...
                    <button type="button" class="tfbtn" data-tf="invoice">BILLS</button>
                    <button type="button" class="tfbtn" data-tf="payment">PAYMENTS</button>
                    <button type="button" class="tfbtn" data-tf="creditnote">CREDIT NOTES</button>
                </div>
            </div>
        @endif
    ...
    @if (!$public)
        <div id="actionsmodal" class="smodal" hidden>
            <div class="sbox">
                ...
                <div style="display:flex; flex-direction:column; gap:8px; margin-top:14px;">
                    ...
                    <a class="tbtn" href="{{ route('creditnotes.create', $partner) }}">RECORD CREDIT NOTE</a>
        ...
        <div id="sharemodal" class="smodal" hidden>
            <div class="sbox">
                ...
                <input type="tel" id="wanum" inputmode="numeric" maxlength="10" value="{{ $partner->mobile }}">
    ...
    <script>
        const IS_PUBLIC = @json($public);
        const DATA_URL = @json($dataUrl);
        const PUBLIC_URL = @json($publicUrl);
        const PARTNER_ID = @json($partner->id);
        @if (!$public)
        const PAY_URL = "{{ route('payments.store') }}";
        @endif
        ...
        function render(full) {
            ...
            if (state.from) {
                rows += '<tr class="open"><td></td><td>Opening balance</td><td class="num"></td><td class="num"></td><td class="num">' + money(d.opening) + '</td></tr>';
                cardhtml += '<div class="lcard"><div class="r1"><span>Opening balance</span><span>' + money(d.opening) + '</span></div></div>';
            }

            d.entries.forEach(e => {
                const label = (!IS_PUBLIC && e.url)
                    ? '<a href="' + e.url + '">' + esc(e.label) + '</a>'
                    : esc(e.label);
                rows += '<tr>'
                    + '<td>' + fmtDate(e.date) + '</td>'
                    + '<td>' + label + (e.detail ? '<div class="detail">' + esc(e.detail) + '</div>' : '') + '</td>'
                ...
                cardhtml += '<div class="lcard">'
                    + (e.debit > 0 ? '<span class="ltag dr">DEBIT</span>' : '<span class="ltag cr">CREDIT</span>')
                    + '<div class="r1"><span>' + label + '</span><span>' + (e.debit > 0 ? money(e.debit) : '- ' + money(e.credit)) + '</span></div>'
            ...
            if (d.entries.length) {
                rows += '<tr class="tot"><td></td><td>Total</td><td class="num">' + money(d.total_debit) + '</td><td class="num">' + money(d.total_credit) + '</td><td class="num">' + money(d.closing) + '</td></tr>';
            }
            ...
            if (d.closing > 0) {
                ...
                bal.innerHTML = 'Balance due from ' + esc(d.partner.firm_name) + ': <b>' + money(d.closing) + '</b>';
            } else if (d.closing < 0) {
                bal.className = 'balline adv';
                bal.innerHTML = 'Advance held from ' + esc(d.partner.firm_name) + ': <b>' + money(Math.abs(d.closing)) + '</b>';
            } else {
        ...
        function openPay() {
            const t = new Date();
            document.getElementById('p_date').value = t.getFullYear() + '-' + String(t.getMonth() + 1).padStart(2, '0') + '-' + String(t.getDate()).padStart(2, '0');
